...
import { useExplorer } from '@icecreamswap/bridge-ui-core'
import { TransferDetailView } from '../../components'
import { fetchTransaction } from '@icecreamswap/bridge-ui-core'
import { computeTransferDetails } from '../../utils/Helpers'
import { DepositRecord, TransferDetails } from '@icecreamswap/bridge-ui-core'
...
const TransactionPage: React.FC<AppProps> = () => {
  const {
    __RUNTIME_CONFIG__: {
  ...
  const [delay, setDelay] = useState<null | number>(transactionAutoUpdateInterval ?? 5000)
  const [transferDetailed, setTransferDetailed] = useState<TransferDetails | null>(null)
  ...
  useEffect(() => {
    if (transaction && Object.keys(transaction).length) {
      const txDetail = computeTransferDetails(transaction as DepositRecord, chains)
  ...
  return (
    <div className={classes.transferDetailViewContainer}>
      {transferDetailed && <TransferDetailView transferDetails={transferDetailed!} />}
    </div>
  )
}
