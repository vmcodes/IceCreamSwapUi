...
import { useChainbridge, useNetworkManager } from '@icecreamswap/bridge-ui-core'
...
import { WrapTokenInput, WrapTokenSelectInput, TokenBalance } from '../../components'
...
const MainPage = () => {
  ...
  const { wrapTokenConfig, wrapToken, unwrapToken, homeConfig, isReady, tokens, nativeTokenBalance, address } =
    useChainbridge()
  ...
  const { handleSubmit, control, setValue, watch, formState } = useForm<PreflightDetails>({
  ...
  const tokenBalance = action === 'wrap' ? nativeTokenBalance : tokens[wrapTokenConfig?.address || '0x']?.balance
  ...
  return (
    <div className={classes.root}>
            ...
            <WrapTokenSelectInput
              ...
              options={[
                ...
                },
                {
                  label: (
                    <div className={classes.tokenItem}>
                      <img src={showImageUrl(wrapTokenConfig?.imageUri || 'WETHIcon')} alt={wrapTokenConfig?.symbol} />
      ...
      <PreflightModalWrap
        ...
        tokenSymbol={action === 'wrap' ? homeConfig?.nativeTokenSymbol || 'ETH' : wrapTokenConfig?.symbol || 'wETH'}
